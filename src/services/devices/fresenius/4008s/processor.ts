...
import { FRESENIUS_4008S_CONFIG, FRESENIUS_4008S_UNITS, FRESENIUS_4008S_CATEGORIES } from './config.js';
import { FRESENIUS_4008S_PROMPT } from './prompts.js';

export class Fresenius4008SProcessor {
  private config = FRESENIUS_4008S_CONFIG;
  ...
  async processImage(base64Image: string, context: ProcessingContext): Promise<{
    ...
    console.log('🔧 [Fresenius4008S] Processing image with device-specific logic');
    
    try {
      // Use device-specific prompt
      const prompt = context.processingOptions.useDeviceSpecificPrompt 
        ? FRESENIUS_4008S_PROMPT 
      ...
      console.log('📡 [Fresenius4008S] API Response status:', response.status);
      const headersObj: Record<string, string> = {};
      response.headers.forEach((value, key) => {
        headersObj[key] = value;
      });
      console.log('📡 [Fresenius4008S] API Response headers:', headersObj);

      if (!response.ok) {
        let errorMessage = `OpenAI API Error: ${response.status}`;
        try {
          const responseText = await response.text();
          console.error('❌ [Fresenius4008S] Error response text:', responseText);
          ...
          console.error('❌ [Fresenius4008S] Could not read error response:', textError);
      ...
      try {
        const responseText = await response.text();
        console.log('📡 [Fresenius4008S] Raw response:', responseText.substring(0, 500));
        result = JSON.parse(responseText);
      } catch (parseError) {
        console.error('❌ [Fresenius4008S] JSON parse error:', parseError);
        throw new ProcessingError('PROCESSING_FAILED', 'Invalid JSON response from OpenAI API. Please check your API key and try again.', { parseError }, 'Fresenius4008S');
      }
      ...
      if (safeExtractedText.includes("I'm unable to analyze images directly") || 
        ...
        console.error('❌ [Fresenius4008S] OpenAI API refusing to analyze image');
        throw new ProcessingError('OPENAI_REFUSAL', safeExtractedText, { rawResponse: safeExtractedText }, 'Fresenius4008S');
      }
      ...
      if (jsonMatch) {
        try {
          ...
          console.warn('⚠️ [Fresenius4008S] Failed to parse JSON from API response:', parseError);
          console.log('📄 [Fresenius4008S] Raw content:', safeExtractedText);
          if (safeExtractedText.toLowerCase().includes("sorry") || safeExtractedText.toLowerCase().includes("can't")) {
            throw new ProcessingError('OPENAI_REFUSAL', safeExtractedText, { rawResponse: safeExtractedText }, 'Fresenius4008S');
          }
          throw new ProcessingError('PROCESSING_FAILED', 'OpenAI returned invalid JSON format', { rawResponse: safeExtractedText.substring(0, 200) }, 'Fresenius4008S');
        }
      } else {
        console.warn('⚠️ [Fresenius4008S] No JSON array found in API response');
        console.log('📄 [Fresenius4008S] Raw content:', safeExtractedText);
        if (safeExtractedText.toLowerCase().includes("sorry") || safeExtractedText.toLowerCase().includes("can't")) {
          throw new ProcessingError('OPENAI_REFUSAL', safeExtractedText, { rawResponse: safeExtractedText }, 'Fresenius4008S');
        }
        throw new ProcessingError('PROCESSING_FAILED', 'OpenAI did not return expected JSON format', { rawResponse: safeExtractedText.substring(0, 200) }, 'Fresenius4008S');
      }
      ...
      console.log('✅ [Fresenius4008S] Processing completed:', validatedData.length, 'data points');
      
      return {
        data: validatedData,
        processingTime: Math.round(processingTime * 10) / 10,
        modelUsed: 'gpt-4o-fresenius-4008s'
      };
      
    } catch (error) {
      console.error('❌ [Fresenius4008S] Processing failed:', error);
      throw error;
    }
  }
  ...
  async processImageViaBackend(
    ...
    try {
      ...
      return {
        ...result,
        processingTime: Math.round(processingTime * 10) / 10,
        modelUsed: 'backend-fresenius-4008s'
      };
    } catch (error) {
      console.error('❌ [Fresenius4008S] Backend processing failed:', error);
      throw error;
    }
  }
  ...
  private getCategoryForLabel(label: string): DeviceDataPoint['category'] {
    return (FRESENIUS_4008S_CATEGORIES as any)[label] || 'other';
  }
      ...
      if (rule && rule.type === 'range') {
        ...
        if (!isNaN(value)) {
          if (value >= (rule.min || 0) && value <= (rule.max || Infinity)) {
            ...
            console.warn(`⚠️ [Fresenius4008S] ${point.label} value ${value} outside expected range`);
          }
        }
      }
  ...
  getSupportedUnits(label: string): string[] {
    return FRESENIUS_4008S_UNITS[label as keyof typeof FRESENIUS_4008S_UNITS] || [];
  }
}