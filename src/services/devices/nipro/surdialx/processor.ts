...
import { NIPRO_SURDIALX_CONFIG, NIPRO_SURDIALX_UNITS, NIPRO_SURDIALX_CATEGORIES } from './config.js';
import { NIPRO_SURDIALX_PROMPT } from './prompts.js';

export class NiproSurdialXProcessor {
  private config = NIPRO_SURDIALX_CONFIG;
  ...
  async processImage(base64Image: string, context: ProcessingContext): Promise<{
    ...
    console.log('🔧 [NiproSurdialX] Processing image with device-specific logic');
    
    try {
      // Use device-specific prompt
      const prompt = context.processingOptions.useDeviceSpecificPrompt 
        ? NIPRO_SURDIALX_PROMPT 
      ...
      console.log('📡 [NiproSurdialX] API Response status:', response.status);
      const headersObj: Record<string, string> = {};
      response.headers.forEach((value, key) => {
        headersObj[key] = value;
      });
      console.log('📡 [NiproSurdialX] API Response headers:', headersObj);

      if (!response.ok) {
        let errorMessage = `OpenAI API Error: ${response.status}`;
        try {
          const responseText = await response.text();
          console.error('❌ [NiproSurdialX] Error response text:', responseText);
          ...
          console.error('❌ [NiproSurdialX] Could not read error response:', textError);
      ...
      try {
        const responseText = await response.text();
        console.log('📡 [NiproSurdialX] Raw response:', responseText.substring(0, 500));
        result = JSON.parse(responseText);
      } catch (parseError) {
        console.error('❌ [NiproSurdialX] JSON parse error:', parseError);
        throw new ProcessingError('PROCESSING_FAILED', 'Invalid JSON response from OpenAI API. Please check your API key and try again.', { parseError }, 'NiproSurdialX');
      }
      ...
      if (safeExtractedText.includes("I'm unable to analyze images directly") || 
        ...
        console.error('❌ [NiproSurdialX] OpenAI API refusing to analyze image');
        throw new ProcessingError('OPENAI_REFUSAL', safeExtractedText, { rawResponse: safeExtractedText }, 'NiproSurdialX');
      }
      ...
      if (jsonMatch) {
        try {
          ...
          console.warn('⚠️ [NiproSurdialX] Failed to parse JSON from API response:', parseError);
          console.log('📄 [NiproSurdialX] Raw content:', safeExtractedText);
          if (safeExtractedText.toLowerCase().includes("sorry") || safeExtractedText.toLowerCase().includes("can't")) {
            throw new ProcessingError('OPENAI_REFUSAL', safeExtractedText, { rawResponse: safeExtractedText }, 'NiproSurdialX');
          }
          throw new ProcessingError('PROCESSING_FAILED', 'OpenAI returned invalid JSON format', { rawResponse: safeExtractedText.substring(0, 200) }, 'NiproSurdialX');
        }
      } else {
        console.warn('⚠️ [NiproSurdialX] No JSON array found in API response');
        console.log('📄 [NiproSurdialX] Raw content:', safeExtractedText);
        if (safeExtractedText.toLowerCase().includes("sorry") || safeExtractedText.toLowerCase().includes("can't")) {
          throw new ProcessingError('OPENAI_REFUSAL', safeExtractedText, { rawResponse: safeExtractedText }, 'NiproSurdialX');
        }
        throw new ProcessingError('PROCESSING_FAILED', 'OpenAI did not return expected JSON format', { rawResponse: safeExtractedText.substring(0, 200) }, 'NiproSurdialX');
      }
      ...
      console.log('✅ [NiproSurdialX] Processing completed:', validatedData.length, 'data points');
      
      return {
        data: validatedData,
        processingTime: Math.round(processingTime * 10) / 10,
        modelUsed: 'gpt-4o-nipro-surdialx'
      };
      
    } catch (error) {
      console.error('❌ [NiproSurdialX] Processing failed:', error);
      throw error;
    }
  }
  ...
  async processImageViaBackend(
    ...
    try {
      ...
      return {
        ...result,
        processingTime: Math.round(processingTime * 10) / 10,
        modelUsed: 'backend-nipro-surdialx'
      };
    } catch (error) {
      console.error('❌ [NiproSurdialX] Backend processing failed:', error);
      throw error;
    }
  }
  ...
  private isLabelVariant(label: string, targetLabel: string): boolean {
    const variants: Record<string, string[]> = {
      'TEMP': ['Temperature'],
      'COND': ['Conductivity'],
      'PRESCR_NA': ['Prescr. Na', 'Sodium'],
      'PRESCR_BIC': ['Prescr. Bic', 'Bicarbonate'],
      'ART_PRESSURE': ['ART'],
      'VEN_PRESSURE': ['VEN'],
      'MACHINE_ID': ['MC:NO'],
      'BPM_SYSTOLIC': ['BP Systolic', 'Systolic'],
      'BPM_DIASTOLIC': ['BP Diastolic', 'Diastolic'],
      'BPM_PULSE': ['Pulse', 'Heart Rate']
    };
  ...
  private normalizeLabel(label: string): string {
    const normalizations: Record<string, string> = {
      ...
      'Conductivity': 'COND',
      'Prescr. Na': 'PRESCR_NA',
      'Prescr. Bic': 'PRESCR_BIC',
      'Sodium': 'PRESCR_NA',
      'Bicarbonate': 'PRESCR_BIC',
      'ART': 'ART_PRESSURE',
      'VEN': 'VEN_PRESSURE',
      'MC:NO': 'MACHINE_ID',
      'BP Systolic': 'BPM_SYSTOLIC',
      'BP Diastolic': 'BPM_DIASTOLIC',
      'Systolic': 'BPM_SYSTOLIC',
      'Diastolic': 'BPM_DIASTOLIC',
      'Pulse': 'BPM_PULSE',
      'Heart Rate': 'BPM_PULSE'
    };
  ...
  private getCategoryForLabel(label: string): DeviceDataPoint['category'] {
    return (NIPRO_SURDIALX_CATEGORIES as any)[label] || 'other';
  }
      ...
      if (rule && rule.type === 'range') {
        ...
        if (!isNaN(value)) {
          if (value >= (rule.min || 0) && value <= (rule.max || Infinity)) {
            ...
            console.warn(`⚠️ [NiproSurdialX] ${point.label} value ${value} outside expected range`);
          }
        }
      }
  ...
  getSupportedUnits(label: string): string[] {
    return NIPRO_SURDIALX_UNITS[label as keyof typeof NIPRO_SURDIALX_UNITS] || [];
  }
}